...
import 'package:kelime_ezberleme_uyg/db/models/lists.dart';
...
class DB
{
  ...
  Future<List<Map<String,Object?>>> readListAll()async
  {
    ...
    await Future.forEach(lists, (element) async {
      element = (element as Map).cast<String, Object?>();
  ...
  Future<int> deleteWords(int id) async
  {
    final db = await instance.database;
  ...
  Future<int> deleteListAndWordByList(int id) async
  {
    final db = await instance.database;
